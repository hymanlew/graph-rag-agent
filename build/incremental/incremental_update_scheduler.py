...
class IncrementalUpdateScheduler:
    ...
    调度器通过配置化的时间阈值管理，协调各个组件（如文件变更检测、实体嵌入更新、社区检测、图结构完整性验证等）
    的更新时机和频率。确保资源高效利用的同时保持数据最新。
    
    架构设计：
    - 基于时间阈值的调度决策系统 - 维护各组件的更新时间记录和阈值配置
    - 支持定时更新和按需更新两种模式 - 灵活应对不同场景需求
    - 任务管理系统 - 负责调度、执行和监控更新任务，通过配置文件支持自定义调度策略
    - 线程安全机制 - 使用线程锁和事件机制确保并发安全
