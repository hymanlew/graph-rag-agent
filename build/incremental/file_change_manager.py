...
此模块负责在Graph-RAG系统中追踪和管理文件的变更状态，是增量更新系统的核心组件。
通过维护文件注册表，记录文件的哈希值和元数据，可以高效识别新增、修改和删除的文件，
为增量更新提供准确的变更信息，避免不必要的重复处理，提升系统性能。

增量更新是Graph-RAG系统的关键性能优化技术，允许系统只处理发生变化的文档，而不是
每次都重建整个知识图谱。这种方式显著减少了处理时间和计算资源消耗，特别是在文档集合
较大且更新频率较高的情况下。

文件变更管理器通过基于内容的哈希比较来精确识别文件变化，相比仅依赖文件修改时间等
元数据的方法更加可靠。同时，它还维护了丰富的文件处理历史记录，便于性能分析和系统调试。

主要功能点：
- 文件哈希值计算和存储 - 基于SHA256算法的内容标识
- 文件变更检测（新增、修改、删除）- 三向对比的高效变更识别
- 文件元数据管理 - 记录文件大小、修改时间等重要属性
- 处理历史记录追踪 - 维护详细的文件处理历史
- 增量更新支持 - 为整个增量更新流程提供基础数据

该模块与增量更新调度器和手动编辑管理器协同工作，共同构成了Graph-RAG系统的增量
更新核心框架。
"""
