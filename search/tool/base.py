...
class BaseSearchTool(ABC):
    ...
    def db_query(self, cypher: str, params: Dict[str, Any] = {}):
        ...
        """
        # 使用连接管理器执行查询
        return self.db_manager.execute_query(cypher, params)
    ...
    def vector_search(self, query: str, limit: int = 5) -> List[str]:
        """
        基于向量相似度的搜索方法
        将文本查询转换为向量表示，然后在向量索引中查找语义相似度最高的实体
    ...
    def text_search(self, query: str, limit: int = 5) -> List[str]:
        ...
        技术特点：
        - 关键词匹配：基于简单的字符串包含关系进行匹配，CONTAINS操作符
    ...
    def semantic_search(self, query: str, entities: List[Dict], 
        ...
        负责计算查询与一组预检索实体之间的语义相似度，并根据相似度对实体进行排序。
        与基础向量搜索不同，它针对已经检索到的特定实体集合进行精细化排序，
        显著提高了搜索结果的针对性和相关性。
    ...
    def filter_by_relevance(self, query: str, docs: List, top_k: int = 5) -> List:
        """
        根据相关性过滤文档
        根据查询与文档的语义相似度对文档列表进行排序和筛选。通过计算语义相关性，能够从大量文档中找出最相关的内容。
