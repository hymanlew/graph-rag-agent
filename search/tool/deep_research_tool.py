...
该模块实现了一个强大的深度研究工具，通过多步骤的思考-搜索-推理过程，
能够解决复杂问题并提供深入的分析结果。这是Graph-RAG系统中最高级的
搜索工具，整合了多种搜索策略和推理机制。
...
设计思路：
- 采用迭代式深度搜索策略，逐步深入分析问题
- 结合多种搜索工具的优势，提供全面的信息检索
- 实现结构化思考过程，记录完整的推理步骤
- 支持流式输出和实时反馈，提升用户体验
- 包含完善的异常处理和降级机制，确保系统稳定性
- 提供答案质量验证，确保生成内容的准确性和相关性
...
- 使用HybridSearchTool进行关键词提取和混合搜索
- 使用GlobalSearchTool获取社区和主题信息
- 使用LocalSearchTool进行知识库检索
- 集成ThinkingEngine实现结构化思考过程
- 使用DualPathSearcher整合知识库和知识图谱搜索
...
class DeepResearchTool(BaseSearchTool):
    ...
    该工具实现了复杂的多步骤思考-搜索-推理循环，通过迭代方式逐步深入分析问题，
    能够解决需要多维度信息、逻辑推理和深入分析的复杂问题。
    
    设计思路：
    - 采用迭代式深度搜索策略，逐步深入探索问题的各个方面
    - 结合多种搜索工具的优势，整合不同类型的信息源
    - 实现结构化思考过程，记录完整的推理步骤
    - 支持流式输出和实时反馈，提升用户体验
    - 包含完善的异常处理和降级机制，确保系统稳定性
    - 提供答案质量验证，确保生成内容的准确性和相关性
    ...
    核心功能：
    1. 问题分解与子查询生成：将复杂问题分解为可管理的子问题
    2. 多轮迭代思考与搜索：通过迭代过程逐步深入问题
    3. 信息提取与整合：从搜索结果中提取有价值的信息
    4. 双路径搜索：同时查询知识库和知识图谱，获取全面信息
    5. 流式响应和实时反馈：提供思考过程的实时展示
    6. 答案生成与验证：生成高质量答案并验证其准确性
    7. 性能监控与日志记录：跟踪执行过程和性能指标
    
    实现策略：
    - 组件化设计：集成多种专用工具完成不同任务
    - 缓存机制：减少重复计算，提高性能
    - 自适应搜索：根据已收集的信息动态调整搜索策略
    - 优雅降级：当主要搜索方法失败时提供备选方案
    - 异步支持：提供同步和异步两种接口，适应不同场景
    ...
    与其他工具的协同：
    - 与HybridSearchTool协作进行关键词提取
    - 与GlobalSearchTool协作获取社区信息
    - 与LocalSearchTool协作进行知识库检索
    - 与ThinkingEngine协作管理思考过程
    - 与DualPathSearcher协作执行双路径搜索
    """
    ...
    def __init__(self):
        ...
        该方法是DeepResearchTool的初始化函数，负责构建整个深度研究系统的核心组件和依赖关系。
        它通过组合多种专业工具和引擎，创建一个完整的深度研究框架，支持复杂问题的分析和解答。
        初始化过程涉及组件配置、缓存机制设置、工具整合和参数优化等多个方面。
        
        实现思路：
        1. 调用父类BaseSearchTool构造函数，设置特定的缓存目录，优化性能
        2. 初始化关键词缓存字典，避免重复提取相同查询的关键词，提高效率
