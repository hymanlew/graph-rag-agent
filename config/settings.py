...
# 知识库分类设置（定义整个知识库的领域范围），用于deepsearch（reasoning提示词），指导系统的语义理解方向
KB_NAME = "华东理工大学"

# fastapi 系统运行并发进程数，控制API服务器的并发处理能力
...
# 知识图谱主题设置（知识库中的某一个知识图谱的主题），定义整个知识图谱的领域范围
...
# 冲突解决与更新策略（增量更新策略）
# 定义了当新文件自动更新与手动编辑图数据库，发生冲突时的处理方式
...
# 社区检测算法配置
# 社区检测，是用于发现知识图谱中的紧密相关实体群组的算法。可选值：'leiden'或'sllpa'
# Leiden 算法通常在大型图上表现更好，而 SLLPA 在特定结构的图上可能更高效
...
# ===== 性能优化配置 =====
# 控制系统的并行处理能力和批处理大小，是系统性能调优的关键参数，需要根据硬件资源进行合理配置。
...
# ===== 搜索模块配置 =====
# 定义了系统的搜索策略，包括局部搜索（文本块扩展到社区）和全局搜索（社区到文本块）两种主要模式，

# 本地搜索配置，用于基于图的局部搜索。直接检索 与查询直接相关的实体和关系
LOCAL_SEARCH_CONFIG = {
    # 向量检索参数，控制检索结果的数量和质量
    ...
    "index_name": "vector",       # 向量索引配置，向量索引名称，用于快速检索
...
# 全局搜索配置，用于基于图的全局搜索。针对整个知识图谱进行广度检索，发现潜在关联
...
# ===== 缓存配置 =====
# 通过多级缓存机制优化搜索性能，减少重复计算和数据库访问，提升响应速度。
