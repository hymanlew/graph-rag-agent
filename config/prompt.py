...
-entity_name：实体名称，大写 
-entity_type：以下类型之一：[{entity_types}]  # 动态插入用户定义的实体类型
...
# 实体类型提取提示模板
system_template_extract_entity_types="""
# 实体类型识别系统提示
# 用途：从文本中自动发现并归纳出关键实体类型，为知识图谱构建提供基础类别

-目标- 
分析提供的文本片段，识别可能的实体类型列表，这些实体类型对于理解文本内容至关重要。 

-步骤- 
1.通读整个文本片段，了解文本的主要内容和主题。 
2.识别文本中提到的关键概念、人物、组织、地点、事件等。 
3.将这些关键概念分类，归纳出主要的实体类型。 
4.确保生成的实体类型列表全面覆盖文本中的重要实体，同时避免过于具体或过于笼统的类别。 
...
# 朴素检索提示模板
# 用途：基于检索到的文档片段直接生成回答，是RAG系统中最基本的回答生成方式
NAIVE_PROMPT="""
---角色--- 
...
# LangChain系统提示模板
# 用途：用于综合多个不同类型的分析报告（实体、关系、文档片段等）生成全面回答
LC_SYSTEM_PROMPT="""
---角色--- 
...
# Map阶段提示模板 - 用于从图社区中提取关键信息
# 在RAG系统的MapReduce架构中，此模板用于从每个图社区中提取与查询相关的关键要点
MAP_SYSTEM_PROMPT = """
---角色--- 
...
# Reduce阶段提示模板 - 用于合并多个社区的信息并生成最终答案
# 在RAG系统的MapReduce架构中，此模板用于将多个图社区提取的要点合并为一个综合的最终回答
...
# 上下文问题转换提示模板
# 用途：将依赖上下文的对话问题转换为独立可理解的问题，便于后续的检索和回答
